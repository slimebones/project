...
{ind}6. not starting with a digit"""
    ...
    def call(self, command: str, callback: Callable[[int, str], None] | None = None):
        ...
        loc = self.project.source

        # Use nushell command.
        if "\"" in command:
            self.response(f"{colorama.Fore.YELLOW}WARNING{colorama.Fore.RESET}: Please, do not use double-quotes for 'call' commands - they will be replaced with single-quotes. Occurred for project '{self.project.id}', command: {command}")
    ...
    def code(self, target: PathLike):
        ...
        target = Path(self.project.source, target)
...
{codenames}}};""".format(codenames=codenames)
    ...
    def include(self, target: Path | str, dest: Path | str | None = None):
        ...
        """
        target = Path(target)
        real_target = Path(self.project.source, target)
